...
plot_predictions <- function(model, nLetters.WD, groups_vec, group_means = NULL,
                             ...
                             labels_custom_yes = FALSE, labels_custom = NULL) {
  ...
  mean_dat <- model_dat %>% filter(group %in% groups_vec) %>%
    group_by(group) %>% dplyr::summarize(nLetters.WD = mean(nLetters.WD)) %>%
  ...
  pred_dat <- pred_dat %>%
    select(group, Estimate, StdErr, nLetters.WD_obs, nLetters.WD_fix, pred, pred_StdErr, ci_lower, ci_upper)
  ...
  colnames(pred_dat)[colnames(pred_dat) == "ci_lower"] <- paste0(response_short, ".ci_lower")
  colnames(pred_dat)[colnames(pred_dat) == "ci_upper"] <- paste0(response_short, ".ci_upper")
  pred_dat <- as.data.frame(pred_dat)
  row.names(pred_dat) <- pred_dat$group
  pred_dat$group <- NULL
  pred_dat <- as.matrix(pred_dat)
  #print(row.names(pred_dat))
  #print(groups_vec)
