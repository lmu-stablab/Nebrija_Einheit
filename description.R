...
# read data
dat_list <- lapply(files, function(file) {
  readxl::read_excel(paste0(path_rawData, file), sheet = 1, na = ".") %>%
    mutate(across(everything(), ~ replace_na(., 0)))
})
# select only AOIs with CU or PU (Experiment 2)
dat_list[[1]] <- dat_list[[1]][grepl("CU|PU", dat_list[[1]]$AOI_LABEL), ]
#------------------------------
...
dat <- dat %>%
  dplyr::rename("TRT"      = "AOI_TOTAL_READING_TIME",
                ...
                "FFD"      = "AOI_FIRST_FIXATION_DURATION",
                "FC"       = "AOI_FIXATION_COUNT",
...
vars <- c(paste0(rep(c(response_vec[-4]), times = 2), rep(c("",".WD"), each = length(response_vec))), "FFD")
for (var in vars)
...
for (var in response_vec) {
  ...
  } else if (any(dat[,paste0(var,".WD")] < 0, na.rm = TRUE))
    error_messages <- append(error_messages, paste0("--- Negative values exist in ",var,"!"))
}
if (length(error_messages) == 0)
...
vars <- c("AOI", paste0(response_vec[-4], ".WD"), "FFD")
